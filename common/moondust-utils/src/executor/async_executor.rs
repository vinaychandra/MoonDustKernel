//! Ported from https://github.com/smol-rs/async-executor/blob/master/src/lib.rs
//! Async executors.
//!
//! # Examples
//!
...
impl<'a> Executor<'a> {
    ...
    /// A forever executor that yields every now and then upto a max of
    /// n loops of runnables.
    pub async fn run_n_loops(&self, loops: usize) {
        let runner = Runner::new(self.state());
...
struct State {
    /// The global queue.
    queue: BoundedSegQueue<Runnable>,

    /// Local queues created by runners.
    local_queues: RwLock<Vec<Arc<BoundedSegQueue<Runnable>>>>,
...
impl State {
    ...
    fn new() -> State {
        State {
            queue: BoundedSegQueue::unbounded(),
...
struct Runner<'a> {
    ...
    /// The local queue.
    local: Arc<BoundedSegQueue<Runnable>>,
...
impl Runner<'_> {
    ...
    fn new(state: &State) -> Runner<'_> {
        let runner = Runner {
            ...
            local: Arc::new(BoundedSegQueue::bounded(512)),
...
fn steal<T>(src: &BoundedSegQueue<T>, dest: &BoundedSegQueue<T>) {
