...
impl<T: Ord + Clone> Node<T> {
    ...
    fn height(node: &Option<Arc<Node<T>>>) -> usize {
        match node {
            None => 0,
            Some(n) => n.height,
        }
    ...
    fn remove(&self, interval: &Interval<T>) -> Option<Arc<Self>> {
        let res = if interval == &self.interval {
            ...
            match &self.left {
                ...
                Some(left_tree) => Some(Arc::new(self.replace_left(left_tree.remove(interval)))),
            }
        } else {
            match &self.right {
                None => Some(Arc::new(self.clone())),
                Some(right_tree) => Some(Arc::new(self.replace_right(right_tree.remove(interval)))),
            }
        };
        match res {
            None => None,
            Some(r) => Some(Arc::new(r.balance())),
        }
    }

    fn replace_left(&self, new_left: Option<Arc<Node<T>>>) -> Node<T> {
        Self::new(self.interval.clone(), new_left, self.right.clone())
    }

    fn replace_right(&self, new_right: Option<Arc<Node<T>>>) -> Node<T> {
    ...
    fn balance(&self) -> Self {
        ...
        if balance_factor < -1 {
            ...
            if right.balance_factor() > 0 {
                self.replace_right(Some(Arc::new(right.rotate_right())))
                    .rotate_left()
            } else {
                self.rotate_left()
            }
            ...
            if left.balance_factor() < 0 {
                self.replace_left(Some(Arc::new(left.rotate_left())))
                    .rotate_right()
            } else {
                self.rotate_right()
            }
