...
pub struct Hpet {
    ...
    /// The number of timers of this HPET device.
    timer_count: u32,
}
...
impl Hpet {
    ...
    pub fn new(inner: *mut HpetRegister) -> Self {
        ...
        res.timer_count = res.get_inner().identifier.read().timer_count_minus_one() + 1;
        res.period = res.get_inner().period.read();
    ...
    pub fn has_legacy_mapping(&self) -> bool {
        self.get_inner().identifier.read().legacy_rt_capability()
    }
    ...
    pub fn enable_legacy_mapping(&self) {
        let mut general_configuration = self.get_inner().general_configuration.read();
    ...
    pub fn is_legacy_mapping_enabled(&self) -> bool {
        let general_configuration = self.get_inner().general_configuration.read();
    ...
    pub fn set_main_counter_value(&self, value: u64) {
        self.get_inner().main_counter_value.write(value)
    }

    /// Get HPET main counter value.
    pub fn get_main_counter_value(&self) -> u64 {
        self.get_inner().main_counter_value.read()
    }
    ...
    fn get_inner(&self) -> &mut HpetRegister {
